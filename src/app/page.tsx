...
const VERSION = "3.0.1-PARALLELOGRAM";
...
function AppShell() {
  ...
  }, [tasks, quotes, notes, messages, hasMounted]);
  ...
  const handleCloudSync = async (direction: 'PUSH' | 'PULL') => {
    ...
    try {
        if (direction === 'PUSH') {
            const ledger = { tasks, quotes, notes, messages, lastSync: new Date().toISOString() };
            ...
            if (res.ok) {
                ...
                setTasks(data.tasks || []); setQuotes(data.quotes || []); setNotes(data.notes || []); setMessages(data.messages || []);
                alert("Sovereign Ledger Hydrated from Cloud.");
            }
        }
      ...
      <div className="relative z-10 h-full w-full flex flex-col hud-panel border-white/10 overflow-hidden shadow-2xl bg-black/40 backdrop-blur-md rounded-none md:rounded-lg">
        <header className="h-14 md:h-16 border-b border-white/10 bg-black/80 flex items-center justify-between px-4 md:px-10 shrink-0 relative">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => setActiveModule('BRIDGE')}>
             <div className="h-10 w-10 md:h-12 border-2 border-accent/40 bg-black flex items-center justify-center relative shadow-[0_0_15px_rgba(0,212,255,0.2)]">
                <span className="system-text text-xl font-black text-accent">T</span>
                <div className="bracket-tl" /><div className="bracket-br" />
             </div>
             <div className="flex flex-col text-left">
                <span className="system-text text-[10px] font-black tracking-[0.4em] text-white/90 uppercase font-black">The Bridge</span>
          ...
          <div className="flex gap-2">
            ...
            >
                ...
                <div className="flex flex-col text-left uppercase">
                    <span className="system-text text-[8px] text-white font-black italic tracking-widest leading-none uppercase">{isSystemLinked ? 'Mothership_Stable' : 'Identity_Unlinked'}</span>
                  ...
                  <div className={`flex-grow hud-panel flex flex-col overflow-hidden relative shadow-2xl transition-colors duration-1000 ${isProtocolActive ? 'bg-black/80 border-neon-green/20' : 'bg-black/60 border-white/10'}`}>
                     ...
                     <div className="p-3 bg-black/80 border-t border-white/10 flex flex-col gap-2 uppercase">
                        <div className="flex gap-2 items-center">
                          ...
                          <button onClick={() => handleSend()} className="bg-accent px-4 py-2 system-text text-[9px] font-black tracking-widest uppercase text-white">ENGAGE</button>
                        </div>
                     </div>
                  </div>
                </div>
              ...
              {activeModule === "SETTINGS" && (
                <div className="module-enter h-full overflow-y-auto uppercase flex flex-col items-center py-12 text-left">
                  <div className="w-full max-w-2xl bg-black/40 border border-white/10 p-8 rounded-lg shadow-2xl relative">
                    ...
                    <div className="mt-12 pt-8 border-t border-white/5 flex flex-col gap-4 text-left"><button onClick={() => { localStorage.clear(); window.location.reload(); }} className="w-full py-3 border border-red-500/20 text-red-500/40 text-[9px] font-black hover:bg-red-500 hover:text-white transition-all tracking-[0.3em]">MASTER_OS_RESET</button></div>
                    <div className="bracket-tl" /><div className="bracket-br" />
                  </div>
                </div>
              )}
        ...
        <footer className="h-12 border-t border-white/10 bg-black/95 flex items-center justify-start md:justify-center px-4 overflow-x-auto scrollbar-none gap-1 md:gap-2 shrink-0 z-50">
              {[
                ...
                <div key={item.id} className="relative group">
                    <button
                        ...
                        className={`nav-parallelogram px-6 py-1.5 system-text text-[8px] font-black transition-all border relative overflow-hidden ${activeModule === item.id ? 'text-white border-accent nav-active-pulse' : 'text-white/20 border-white/10 hover:border-white/40 hover:text-white/60'}`}
                    >
                        <span className="nav-text-fix relative z-10 block whitespace-nowrap uppercase">{item.label}</span>
                    </button>
                </div>
              ))}
        </footer>
