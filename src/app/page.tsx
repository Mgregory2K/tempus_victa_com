  ...
  if (status === "loading") {
    return (
      <div className="h-screen w-screen bg-black flex items-center justify-center">
        <p className="system-text text-accent animate-pulse">INITIALIZING NEURAL LINK...</p>
      </div>
    );
  }
  return <AppShell />;
...
function AppShell() {
  ...
  const handleSend = async (overrideInput?: string) => {
    ...
    if (text.toLowerCase() === 'end protocol') {
      ...
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'system', content: 'Ready Room Protocol Terminated.', timestamp: new Date().toISOString() }]);
      setInput("");
      return;
    }
  ...
  const confirmTRRP = () => {
    ...
    setTimeout(() => {
      ...
      setMessages(prev => [...prev, {
        id: Date.now().toString(),
        role: "assistant",
        content: `**PROTOCOL ACTIVE**\n\nTopic: ${p.topic}\nModerator: ${p.moderator}\nMembers: ${p.members}\n\nBegin deliberation. (Type "end protocol" to exit)`,
        timestamp: new Date().toISOString()
      }]);
    }, 4000);
  };
  ...
  const abortTRRP = () => {
    setMatrixPending(false);
    setMessages(prev => [...prev, { id: Date.now().toString(), role: "system", content: "Protocol aborted by user.", timestamp: new Date().toISOString() }]);
  }
    ...
    <div className="relative h-screen w-screen overflow-hidden bg-black text-white p-2 selection:bg-accent/30 font-sans uppercase">
      ...
      <div className="relative z-10 h-full w-full flex flex-col hud-panel border-white/10 overflow-hidden shadow-2xl bg-black/40 backdrop-blur-md">
        <header className="h-16 border-b border-white/10 bg-black/80 flex items-center justify-between px-10 shrink-0 relative uppercase">
          ...
          <div className={`flex items-center gap-6 px-8 py-2 border-2 ${isSystemLinked ? 'border-neon-green/40 bg-neon-green/5' : 'border-red-500/40 bg-red-500/5'} transition-all uppercase`}>
             <div className={`h-2.5 w-2.5 rounded-full ${isSystemLinked ? 'bg-neon-green shadow-[0_0_15px_#22c55e]' : 'bg-red-500'} animate-pulse`} />
          ...
          <main className="flex-grow overflow-hidden relative">
             <div className="absolute inset-0 p-4 overflow-y-auto">
                        ...
                        {messages.map((m) => (
                          <div key={m.id} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'} max-w-full animate-slide-up`}>
                            ...
                            <div className={`max-w-[95%] md:max-w-[95%] p-5 border shadow-xl relative group ${m.role === 'user' ? 'glass-accent border-accent/20 rounded-2xl rounded-tr-sm text-right' : 'glass border-white/10 rounded-2xl rounded-tl-sm text-left bg-white/[0.01]'}`}>
                                <p className={`text-white/90 leading-relaxed whitespace-pre-wrap text-[14px] font-light ${m.role === 'assistant' ? 'italic' : ''}`}>{m.content}</p>
                                {m.role === 'assistant' && (
                                    <div className="mt-4 pt-4 border-t border-white/5 flex items-center justify-between">
                                        <div className="flex gap-4">
                                            <button onClick={() => handleFeedback(m.id, 'up')} className="text-white/20 hover:text-neon-green transition-colors"><svg className="h-4 w-4" fill="currentColor" viewBox="0 0 20 20"><path d="M2 10.5a1.5 1.5 0 113 0v6a1.5 1.5 0 01-3 0v-6zM6 10.333v5.43a2 2 0 001.106 1.79l.05.025A4 4 0 008.943 18h5.416a2 2 0 001.962-1.608l1.2-6A2 2 0 0015.56 8H12V4a2 2 0 00-2-2 1 1 0 00-1 1v.667a4 4 0 01-.8 2.4L6.8 7.933a4 4 0 00-.8 2.4z" /></svg></button>
                                            <button onClick={() => handleFeedback(m.id, 'down')} className="text-white/20 hover:text-red-500 transition-colors"><svg className="h-4 w-4 transform rotate-180" fill="currentColor" viewBox="0 0 20 20"><path d="M2 10.5a1.5 1.5 0 113 0v6a1.5 1.5 0 01-3 0v-6zM6 10.333v5.43a2 2 0 001.106 1.79l.05.025A4 4 0 008.943 18h5.416a2 2 0 001.962-1.608l1.2-6A2 2 0 0015.56 8H12V4a2 2 0 00-2-2 1 1 0 00-1 1v.667a4 4 0 01-.8 2.4L6.8 7.933a4 4 0 00-.8 2.4z" /></svg></button>
                                        </div>
                                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                                            {["Too verbose", "Wrong source", "Not my voice"].map(chip => (<button key={chip} onClick={() => handleFeedback(m.id, chip)} className="text-[7px] border border-white/10 px-2 py-0.5 rounded-full text-white/40 hover:border-accent hover:text-white uppercase">{chip}</button>))}
                                        </div>
                                    </div>
                                )}
                            </div>
              ...
              {activeModule === "DOCTRINE" && (
                <div className="module-enter h-full overflow-y-auto pr-2 scrollbar-thin uppercase">
                  <div className="flex flex-col items-center text-center p-12 max-w-7xl mx-auto uppercase">
                     <h2 className="text-5xl font-black italic tracking-tighter uppercase mb-8 text-white tracking-tighter">THE DOCTRINE</h2>
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full text-left uppercase">
                        {[{ t: "LOCAL", d: "Sovereign knowledge. Persistent, private, deterministic.", s: "ACTIVE" }, { t: "INTERNET", d: "Live factual validation via WiFi/Cellular escalation.", s: isOnline ? "STABLE" : "OFFLINE" }, { t: "AI", d: "Opt-in neural modeling.", s: apiKey ? "ACTIVE" : "STANDBY" }].map(d => (
                          <div key={d.t} className="hud-panel p-8 flex flex-col gap-4 group hover:border-accent/50 transition-all bg-black/60 shadow-2xl relative uppercase"><span className="system-text text-xl font-black italic text-white group-hover:text-accent tracking-tighter uppercase">{d.t}</span><p className="text-[13px] text-white/50 font-light leading-relaxed uppercase">{d.d}</p><div className="pt-4 border-t border-white/5"><span className="system-text text-[8px] text-white/30 font-black uppercase">{d.s}</span></div></div>
                        ))}
                     </div>
                  </div>
                </div>
              ...
              {activeModule === "SETTINGS" && (
                <div className="module-enter h-full overflow-y-auto pr-2 scrollbar-thin uppercase text-left">
                  <div className="flex flex-col max-w-3xl mx-auto py-12 w-full uppercase">
                     <h2 className="text-4xl font-black italic tracking-tighter uppercase mb-10 text-white uppercase">CONFIG</h2>
                     <div className="space-y-12 uppercase">
                        <div className="flex flex-col gap-4 uppercase"><label className="system-text text-[10px] text-white/50 font-black tracking-widest uppercase">Neural Key (OpenAI)</label><div className="flex gap-4 uppercase"><input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="sk-..." className="flex-grow hud-panel bg-black/60 border-white/10 px-4 py-3 text-sm focus:border-accent/50 font-mono text-white shadow-inner uppercase" /><button onClick={() => { localStorage.setItem("tv_api_key", apiKey); alert("Stored"); } } className="bg-accent px-10 py-2 system-text text-[10px] font-black text-white hover:bg-white hover:text-black uppercase">Store</button></div></div>
                        <div className="flex flex-col gap-4 uppercase"><label className="system-text text-[10px] text-white/50 font-black tracking-widest uppercase">Search Key (Tavily)</label><div className="flex gap-4 uppercase"><input type="password" value={searchKey} onChange={(e) => setSearchKey(e.target.value)} placeholder="API Key..." className="flex-grow hud-panel bg-black/60 border-white/10 px-4 py-3 text-sm focus:border-accent/50 font-mono text-white shadow-inner uppercase" /><button onClick={() => { localStorage.setItem("tv_search_key", searchKey); alert("Stored"); } } className="bg-accent px-10 py-2 system-text text-[10px] font-black text-white hover:bg-white hover:text-black uppercase">Store</button></div></div>
              ...
              {activeModule === "MISSIONS" && (
                <div className="module-enter h-full p-8 max-w-7xl mx-auto w-full overflow-y-auto scrollbar-thin uppercase text-left">
                   <h2 className="text-4xl font-black italic tracking-tighter uppercase mb-12 text-white shadow-sm uppercase">Missions</h2>
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-20 uppercase">
                      <div className="hud-panel p-8 border-accent/30 relative group bg-gradient-to-br from-black to-accent/5 shadow-2xl uppercase"><div className="bracket-tl" /><div className="bracket-br" /><h4 className="system-text text-[10px] text-accent font-black italic mb-6 bg-accent/10 px-3 py-1 rounded-sm uppercase">WRIKE P2</h4><h3 className="text-xl font-black mb-4 text-white uppercase tracking-tight italic uppercase">Ingestion</h3><div className="space-y-2 uppercase">
                      <div className="flex justify-between text-[10px] font-black uppercase tracking-widest uppercase"><span>STATUS</span><span className="text-accent uppercase">72%</span></div><div className="h-1 bg-white/5 w-full"><div className="h-full bg-accent w-[72%]" /></div></div></div>
                   </div>
                </div>
              )}
            </div>
        ...
        <footer className={`h-8 border-t border-white/10 bg-black/95 flex items-center justify-center gap-6 px-10 shrink-0 z-50 uppercase`}>
           <div className="flex items-center gap-1 bg-white/[0.01] border border-white/10 p-0.5 h-5 uppercase">
              {["BRIDGE", "READY_ROOM", "MISSIONS", "DOCTRINE", "SETTINGS"].map(id => (
                <button key={id} onClick={() => setActiveModule(id as Module)} className={`px-6 system-text text-[9px] font-black transition-all relative overflow-hidden group min-w-[140px] h-full ${activeModule === id ? 'text-white bg-accent/20' : 'text-white/20 hover:text-white/60 uppercase'}`}><span className="relative z-10 italic uppercase">{id.replace("_", " ")}</span>{activeModule === id && <div className="absolute bottom-0 left-0 h-0.5 w-full bg-accent shadow-[0_-5px_40px_rgba(0,212,255,0.5)] uppercase" />}</button>
              ))}
           </div>
