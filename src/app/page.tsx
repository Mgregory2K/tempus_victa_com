...
function AppShell() {
  ...
  return (
    <div className="relative h-dvh w-screen overflow-hidden bg-black text-white p-0 md:p-2 selection:bg-accent/30 font-sans uppercase text-[10px] flex flex-col" onMouseMove={onMicDrag} onMouseUp={stopMicDrag} onTouchMove={onMicDrag} onTouchEnd={stopMicDrag}>
      ...
      {showDailyBrief && (
        <div className="fixed inset-0 z-[1000] bg-black/60 backdrop-blur-md flex items-center justify-center p-4 md:p-12 overflow-y-auto animate-slide-up" onClick={() => setShowDailyBrief(false)}>
            <div className="max-w-4xl w-full mx-auto relative" onClick={(e) => e.stopPropagation()}><button onClick={() => setShowDailyBrief(false)} className="absolute top-4 right-4 text-black/20 hover:text-black system-text text-[10px] font-black tracking-widest transition-all uppercase z-50">✕ Dismiss</button><DailyBrief /></div>
        </div>
      )}
