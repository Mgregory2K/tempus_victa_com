...
export default function Corkboard({ externalNotes, setNotes, userName, onPromote, onArchive, onBrainstorm }: CorkboardProps) {
    ...
    return (
        <div className="relative w-full h-full min-h-[700px] md:bg-[#2a1a0a] rounded-xl md:border-8 md:border-[#3d2b1f] md:shadow-2xl overflow-hidden animate-slide-up select-none pb-20">
            ...
            <div className="p-4 md:absolute md:top-6 md:left-8 z-10 flex flex-col gap-4">
                <div>
                    <h1 className="text-3xl md:text-4xl font-black text-white md:text-white/30 italic uppercase tracking-widest leading-none">Corkboard</h1>
                    <p className="system-text text-[10px] text-accent md:text-white/20 font-black tracking-[0.4em] mt-2">Spatial Memory // Volume I, Ch 7</p>
                </div>
            ...
            {/* Manual Entry Modal */}
            {isAdding && (
                <div className="fixed inset-0 z-[2000] md:absolute md:inset-0 bg-black/80 md:bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
            ...
            <div className="flex flex-col md:block gap-6 p-4 md:p-0">
                {activeNotes.map(note => (
                    <div key={note.id}
                         ...
                         className={`relative md:absolute p-6 w-full md:w-64 shadow-2xl cursor-grab active:cursor-grabbing transition-all hover:brightness-110 ${note.color} text-black font-medium text-sm z-20 group rounded-sm md:rounded-none`}
                         style={{
                            left: typeof window !== 'undefined' && window.innerWidth > 768 ? `${note.x}px` : 'auto',
                            top: typeof window !== 'undefined' && window.innerWidth > 768 ? `${note.y}px` : 'auto',
                            transform: typeof window !== 'undefined' && window.innerWidth > 768 ? `rotate(${note.rotation}deg)` : 'none',
                            boxShadow: '10px 10px 25px rgba(0,0,0,0.4)',
                         }}>

                        {/* Tactical Menu */}
                        <div className="absolute -top-2 -right-2 md:-top-2 md:-right-2 flex gap-2 md:gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-40">
                            <button onClick={() => onBrainstorm?.(note.text)} className="h-10 w-10 md:h-8 md:w-8 bg-accent text-black rounded shadow-lg flex items-center justify-center hover:scale-110 transition-transform" title="Brainstorm with J5">
                                <svg className="h-5 w-5 md:h-4 md:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg>
                            </button>
                            <button onClick={() => onPromote?.(note.id, 'PROJECTS')} className="h-10 w-10 md:h-8 md:w-8 bg-purple-500 text-white rounded shadow-lg flex items-center justify-center hover:scale-110 transition-transform" title="Promote to Project">
                                <svg className="h-5 w-5 md:h-4 md:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                            </button>
                            <button onClick={() => onPromote?.(note.id, 'TODO')} className="h-10 w-10 md:h-8 md:w-8 bg-neon-green text-black rounded shadow-lg flex items-center justify-center hover:scale-110 transition-transform" title="Promote to To-Do">
                                <svg className="h-5 w-5 md:h-4 md:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                            </button>
                            <button onClick={() => onArchive?.(note.id)} className="h-10 w-10 md:h-8 md:w-8 bg-red-600 text-white rounded shadow-lg flex items-center justify-center hover:scale-110 transition-transform" title="Archive Note">
                                <svg className="h-5 w-5 md:h-4 md:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                            </button>
                        </div>
                        ...
                        <p className="font-handwriting leading-tight text-xl md:text-lg">{note.text}</p>
