...
export default function Winboard({ externalTasks = [] }: WinboardProps) {
    const todayWins = externalTasks.filter(t => t.status === 'DONE');

    return (
        <div className="h-full flex flex-col space-y-8 animate-slide-up pb-20 max-w-5xl mx-auto w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {todayWins.length === 0 ? (
                    <div className="col-span-full py-24 text-center border-2 border-dashed border-white/5 rounded-xl bg-white/[0.01]">
                        <p className="system-text text-[12px] text-white/20 font-black tracking-[0.4em] mb-2">NO_WINS_LOGGED_IN_CURRENT_CYCLE</p>
                    ...
                    todayWins.map(win => (
                        <div key={win.id} className="hud-panel p-6 bg-black/40 border-neon-green/10 relative group hover:border-neon-green/40 transition-all">
                            ...
                            <h3 className="text-xl font-bold italic text-white/90 leading-tight mb-6">
                                {win.title}
                            </h3>
